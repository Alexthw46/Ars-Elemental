...
public class UpstreamTile extends BlockEntity implements ITickable {
    ...
    @Override
    public void tick() {
        if (this.level instanceof ServerLevel level && level.getGameTime() % 2 == 0) {
            List<Entity> entityList = level.getEntitiesOfClass(Entity.class, new AABB(getBlockPos(), getBlockPos().above(46)).inflate(1.5), e -> e.isInWater() && !e.isCrouching());
