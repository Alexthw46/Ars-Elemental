...
public class MethodCarianPhalanx extends AbstractCastMethod {
    ...
    public static final Map<UUID, List<CarianPhalanx>> PLAYER_PHALANX_MAP = new HashMap<>();
     ...
     */
    public static void onPhalanxLaunched(CarianPhalanx phalanx) {
        if (phalanx.getOwner() != null) {
            UUID ownerUUID = phalanx.getOwner().getUUID();
            List<CarianPhalanx> phalanxList = PLAYER_PHALANX_MAP.get(ownerUUID);
            if (phalanxList != null) {
    ...
    private void summonPhalanx(Level world, LivingEntity caster, SpellStats stats, SpellContext context, SpellResolver resolver) {
        ...
        List<CarianPhalanx> existingPhalanx = PLAYER_PHALANX_MAP.getOrDefault(casterUUID, new ArrayList<>());
        ...
        if (newTotal > MAX_PHALANX_PER_PLAYER) {
            ...
            for (int i = 0; i < toRemove && !existingPhalanx.isEmpty(); i++) {
                CarianPhalanx oldest = existingPhalanx.removeFirst();
                if (!oldest.isRemoved()) {
        ...
        List<CarianPhalanx> newPhalanxList = new ArrayList<>();
        for (int i = 0; i < newTotal; i++) {
            if (reserved[i]) continue; // Loop over the full total to fill the gaps in the indexes
            CarianPhalanx phalanx = new CarianPhalanx(world, caster.getX(), caster.getEyeY(), caster.getZ());
    ...
    private void cleanupRemovedPhalanx(UUID playerUUID) {
        List<CarianPhalanx> phalanxList = PLAYER_PHALANX_MAP.get(playerUUID);
        if (phalanxList != null) {
    ...
    public String getBookDescription() {
        return "Summons floating magical projectiles that orbit around you. When an enemy gets close, they automatically launch and home in on the target. " +
                ...
                "Cannot be used by turrets or other automated casters.";
    }
